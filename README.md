...
- 两种处理方式，依次为：
  - **修正表头**：为写错或含义相同的列填写统一目标列名；按来源字段集中编辑，并支持搜索、多表筛选和输出顺序调整。
  - **列名交集**：只保留每张已选表都存在的列。
- 支持字段去空格、大小写转换、条件筛选、整行去重、输出列排序。
- 支持在数据源列表或字段映射中直接弹窗预览源表、查看合并结果，以及字段差异、疑似错拼、空表头、输出规模等合并前检查。
...
- 输出区域可在两种目标间直接切换：
  ...
  - **PostgreSQL**：把完成表头修正或列名交集、清洗、去重后的同一行流导入数据库。
...
3. 在“合并规则”选择修正表头或列名交集，并按需要设置筛选、去重、清洗和输出顺序。
4. 在数据源列表或字段映射中随时预览源表；在“输出预览”查看结果前 30 行和检查报告。
...
src-tauri/src/
  ...
  model.rs            合并、清洗和字段映射模型
  inspect.rs          源数据/结果预览和检查报告
